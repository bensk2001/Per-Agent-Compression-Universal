...
if grep -E "(--model|--channel|--to)" install.sh | grep -v "model:\s*auto\|channel:\s*auto\|to:\s*auto" > /dev/null; then
  echo "    ❌ Install script contains hardcoded params. Fix to use inference."
  exit 1
else
